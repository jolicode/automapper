...
- Add a `json` side to the mappers, naming the JSON document itself symmetrically in both
  directions: a `json` -> class mapper reads a JSON stream, a class -> `json` mapper writes one.
  Both are only reachable through the JsonStreamer integration, `map()` still refuses them
