        ...
        $rootNode
            ->children()
                ...
                ->enumNode('constructor_strategy')
                    ->values(array_map(static fn (ConstructorStrategy $strategy) => $strategy->value, ConstructorStrategy::cases()))
                ...
                ->arrayNode('json_streamer')
                    ->info('Replace the symfony/json-streamer reader and writer with the AutoMapper ones.')
                    ->children()
                        ->booleanNode('enabled')->defaultFalse()->end()
                        ->booleanNode('only_registered_mapping')->defaultFalse()->end()
                    ->end()
                ...
                ->arrayNode('loader')
                    ->children()
                        ...
                        ->enumNode('reload_strategy')->values(array_map(static fn (FileReloadStrategy $value) => $value->value, FileReloadStrategy::cases()))->defaultNull()->end()
                    ->end()
                    ->addDefaultsIfNotSet()
                ->end()
                ->arrayNode('mapping')
