...
final readonly class SerializerIgnoreListener
{
    ...
    public function __invoke(PropertyMetadataEvent $event): void
    {
        ...
        if (($event->mapperMetadata->source !== 'array' && $event->mapperMetadata->source !== \stdClass::class) && $this->isIgnoreProperty($event->mapperMetadata->source, $event->source->property)) {
            $event->ignored = true;
        ...
        if (($event->mapperMetadata->target !== 'array' && $event->mapperMetadata->target !== \stdClass::class) && $this->isIgnoreProperty($event->mapperMetadata->target, $event->target->property)) {
            $event->ignored = true;
